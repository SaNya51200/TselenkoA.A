...
pub fn validate_orders(orders: &[Order], db: &UserDatabase) -> Result<Vec<&Order>, OrderError> {
    ...
    for order in orders {
        match process_order_advanced(db, order) {
            Ok(_) => valid_orders.push(order),
            Err(e) => return Err(e), // Возвращаем первую ошибку
        }
    }
...
pub fn main() {
    ...
    println!("\n=== Практическое задание 2 ===");
    let orders = vec![
        Order { user_id: 1, amount: 99.99, status: "pending".to_string() },
        Order { user_id: 1, amount: 149.99, status: "pending".to_string() },
    ];
    
    match validate_orders(&orders, &user_db) {
        Ok(valid) => println!("Валидные заказы: {}", valid.len()),
