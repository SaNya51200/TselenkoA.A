...
// Практическое задание 1: Анализ продуктов
...
pub fn main() {
    ...
    println!("Количество доступных: {}", available_count);
    println!("Дорогие продукты (>100): {:?}", expensive.iter().map(|p| &p.name).collect::<Vec<_>>());
}
