object ErrorHandling {
  ...
  def processOrderPipeline(order: Order): Either[String, Double] = {
    for {
      user <- findUser(order.userId).toRight(s"User ${order.userId} not found")
      validatedUser <- validateUser(user)
      discount <- calculateDiscount(validatedUser, order.amount)
    } yield discount
  }
  
  def calculateDiscount(user: User, amount: Double): Either[String, Double] = {
    if (amount > 1000) {
      Left("Amount too large for discount")
    } else if (amount > 100) {
      Right(amount * 0.1) // 10% скидка
    } else {
      Right(0.0)
    }
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    val order1 = Order(1, 150.0, "pending")
    val order2 = Order(4, 50.0, "pending") // Несуществующий пользователь
    
    println(s"Заказ 1: ${processOrderPipeline(order1)}")
    println(s"Заказ 2: ${processOrderPipeline(order2)}")
  }
}
